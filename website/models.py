...
class Exclusion(models.Model):
    ...
    def clean(self):
        if (self.mandatory != 'None' or self.discretionary != 'None') and self.exclusion_date is None:
            raise ValidationError('Exclusion date is required if either mandatory or discretionary option is selected.')
        if self.mandatory == 'None' and self.discretionary == 'None' and self.exclusion_date is not None:
            raise ValidationError('Exclusion date is not required if both mandatory and discretionary fields are None.')
